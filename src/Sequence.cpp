...
Sequence::Sequence(SDL_Renderer* renderer, SDL_Window* window, uint8_t boardSize) : _boardSize(boardSize), _renderer(renderer), _window(window)
{
	if (boardSize > 5)
		throw InitFailed();
	SDL_SetWindowSize(_window, (_boardSize + 2) * TILE_SIZE, _boardSize * TILE_SIZE);
...
bool	Sequence::makeMove(int& i, vector2 mouseInput)
{
	...
	renderText(_renderer, str, 4 * TILE_SIZE, TILE_SIZE / 2);
	if (i == _level)
	{
		SDL_Delay(800);
		clearBoard();
	...
	while (_isRunning && _level <= _boardSize * _boardSize)
	{
		...
		while (SDL_PollEvent(&_e))
		{
			...
			if (_e.type == SDL_MOUSEBUTTONDOWN)
			{
				...
				{
					if (makeMove(i, {_e.button.x / TILE_SIZE, _e.button.y / TILE_SIZE}) == false)
						_isRunning = false;
				}
			}
		}
